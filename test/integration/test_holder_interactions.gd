...
class TestHolderWithNoItem extends GutTest:
	...
	func test_player_has_item():
		...
		_player.holder.add_child(holdable)
		
		assert_eq(len(_holder.get_held_items()), 0, "Holder doesn't have 0 items")
		assert_eq(len(_player.holder.get_held_items()), 1, "Player Holder doesn't have 1 items")
		
		_holder.interact(_player)
		
		assert_eq(len(_holder.get_held_items()), 1, "Holder doesn't have 1 items")
		assert_eq(len(_player.holder.get_held_items()), 0, "Player Holder doesn't have 0 items")
...
class TestHolderWithHoldable extends GutTest:
	...
	func test_player_has_item_swaps_with_holder():
		...
		assert_eq(_player.holder.get_held_item(), holdable, "Player not holding Holdable")
		assert_eq(_holder.get_held_item(), holdable_two, "Holder not holding Holdable two")
		
		_holder.interact(_player)

		assert_eq(_player.holder.get_held_item(), holdable_two, "Player not holding Holdable two")
	...
	func test_player_has_multiholder_picks_up_holdable():
		...
		assert_eq(_player.holder.get_held_item(), multi_h, "Player not holding MultiHolder")
		assert_eq(len(_player.holder.get_held_item().holders), 1, "Player's MultiHolder doesn't have 1 Holder")
		assert_eq(len(_player.holder.get_held_item().get_held_items()), 0, "Player's MultiHolder holding something")
		assert_eq(_holder.get_held_item(), holdable, "Holder not holding Holdable")
		
		_holder.interact(_player)

		assert_eq(_player.holder.get_held_item(), multi_h, "Player not holding MultiHolder")
		assert_eq(_player.holder.get_held_item().get_held_item(), holdable, "Player's MultiHolder not holding Holdable")
	...
	func test_player_has_stackingholder_of_same_type_picks_up_holdable():
		...
		_player.holder.add_child(stacking_h)
		_holder.add_child(holdable)
		
		assert_eq(_player.holder.get_held_item(), stacking_h, "Player not holding StackingHolder")
		assert_eq(len(_player.holder.get_held_item().get_held_items()), 0, "Player's StackingHolder has items")
		assert_eq(_holder.get_held_item(), holdable, "Holder not holding Holdable")
		
		_holder.interact(_player)
		
		assert_eq(_player.holder.get_held_item(), stacking_h, "Player not holding the StackingHolder")
		assert_eq(_player.holder.get_held_item().get_held_item(), holdable, "Player not holding the Holdable")
		assert_eq(len(_holder.get_held_items()), 0, "Holder holding something")
	...
	func test_player_has_stackingholder_of_different_type_does_nothing():
		...
		assert_eq(_player.holder.get_held_item(), stacking_h, "Player not holding the StackingHolder")
		assert_eq(len(_player.holder.get_held_item().get_held_items()), 0, "Player holding the Holdable")
...
class TestHolderWithMultiHolder extends GutTest:
	...
	func test_empty_multiholder_player_has_empty_multiholder_does_nothing():
		...
		assert_eq(len(_holder.get_held_item().holders), 1, "Holder's MultiHolder doesn't have 1 Holder")
		assert_eq(len(_holder.get_held_item().get_held_items()), 0, "Holder's MultiHolder holding something")
		
		_holder.interact(_player)

		assert_eq(_player.holder.get_held_item(), multi_h, "Player not holding original MultiHolder")
		assert_eq(len(_player.holder.get_held_item().get_held_items()), 0, "Player's MultiHolder holding something")
...
class TestHolderWithCombinedFoodHolder extends GutTest:
	...
	func test_combinedfood_swaps_with_player_combinedfood():
		...
		_player.holder.add_child(combined_food)
		combined_food.add_child(DoubledFoodClass.new())
		combined_food.add_child(DoubledFoodClass.new())
		combined_food.add_child(DoubledFoodClass.new())
		
		assert_eq(_player.holder.get_held_item(), combined_food, "Player not holding CombinedFood(3)")
		assert_eq(len(_player.holder.get_held_item().get_held_items()), 3, "Player's CombinedFood doesn't have 3 items")
		...
		assert_eq(_player.holder.get_held_item(), _combined_food, "Player not holding CombinedFood(2)")
		assert_eq(len(_player.holder.get_held_item().get_held_items()), 2, "Player's CombinedFood doesn't have 2 items")
